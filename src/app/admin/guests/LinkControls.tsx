...
import { linkRsvp, createPartyFromRsvp } from "./actions";
 ...
 *  2. a "Link to…" dropdown over every party (submits on change),
 *  3. "Add to address book" — create a party from this RSVP and link it.
 */
    ...
    <div className="flex flex-wrap items-center gap-2">
      {suggestion && (
        <form action={linkRsvp} onSubmit={() => setPending(true)}>
          ...
          <input type="hidden" name="party_id" value={suggestion.id} />
          <button
            type="submit"
            disabled={pending}
      ...
      {parties.length > 0 && (
        <form action={linkRsvp} onSubmit={() => setPending(true)}>
          ...
          <select
            ...
            aria-label={`Link ${rsvpName}'s RSVP to a party`}
          >
      ...
      <form action={createPartyFromRsvp} onSubmit={() => setPending(true)}>
        <input type="hidden" name="rsvp_id" value={rsvpId} />
        <button
          type="submit"
          disabled={pending}
